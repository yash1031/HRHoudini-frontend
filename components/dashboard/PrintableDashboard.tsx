...
// Utility: Generate pie colors - Sharp, vibrant colors
const generatePieColors = (count: number): string[] => {
  ...
  // If we need more colors, generate vibrant HSL colors with high saturation
  if (count <= vibrantColors.length) {
    return vibrantColors.slice(0, count);
  }
  
  const colors = [...vibrantColors];
  const step = 360 / (count - vibrantColors.length);
  for (let i = vibrantColors.length; i < count; i++) {
    const hue = (step * (i - vibrantColors.length) + 180) % 360;
    // High saturation (85-95%) and medium brightness (55-65%) for sharp colors
    const saturation = 85 + (i % 10);
    const lightness = 55 + (i % 10);
...
const renderStaticChart = (
  ...
  if (chartConfig.type === 'pie') {
    ...
    // Use larger width to ensure donut chart has enough space
    const chartWidth = Math.max(width, 650);
    const chartHeight = 420;
    // Use percentage-based positioning to ensure proper centering
    const outerRadius = 110;
    const innerRadius = 65; // Donut style
