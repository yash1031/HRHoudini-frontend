...
export default function SelectKPIsPage() {
  ...
  const handleKPIToggle = (kpiId: string) => {
    ...
    setSelectedKPIWithDesc((prev) => {
      const updatedSelected = selectedKPIs.includes(kpiId)
  ...
  const handleNext = async () => {
    ...
    const timeout = setTimeout(() => {
      ...
      console.error("[TIMEOUT] No main charts received within 30 seconds")
    }, 30000)
          ...
          if (msg.event === "kpi.main.ready") {
            ...
            generateChartsFromParquet(mainChartsQueries, parquetUrl)
              .then((result: any) => {
                ...
                // Start timeout for each chart's drilldown
                result.forEach((chart: any) => {
                  const chartId = chart.id || chart.semantic_id
                  console.log("[TIMEOUT] Setting timeout for drilldown of chartId", chartId)
                  const timeout = setTimeout(() => {
                    setDrilldownsState((prev) => ({
                      ...prev,
                      [chartId]: { loading: false, error: true },
                    }))
                    console.error(`[TIMEOUT] No drilldown received for chart ${chartId} within 30 seconds`)
          ...
          if (msg.event === "drilldown_charts.ready") {
            ...
            ;(async () => {
              try {
                ...
                const chartDataResults = await generateDrilldownChartsData(drilldownQueries.charts, parquetUrl)
                ...
                // Merge with existing insights if present
                setChartsState((prev) => {
      ...
      <CardContent>
        ...
        <div className="bg-gray-50 rounded-lg p-3 mb-4">
          <p className="text-sm text-gray-600">
            <strong>Selected:</strong> {selectedKPIs.length} of {kpis.length} KPIs. Your dashboard will show tiles for
            these metrics, and you can add or remove them later.
          </p>
        </div>

        <div className="flex justify-between">
          <Button variant="outline" onClick={() => router.push("/upload/upload-file/success")}>
